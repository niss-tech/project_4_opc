        ...
        if choice == "1":  # Gestion des joueurs
            ...
            while True:
                ...
                if tournament_choice == "1":  # Créer un tournoi
                    ...
                    tournament = next(
                        (t for t in tournament_controller.tournaments if t.name == tournament_name),
                        None
                    )
                        ...
                        if player:
                            ...
                            if player in tournament.players:
                                print(
                                    ...
                                    f"({chess_id}) est déjà inscrit dans ce tournoi."
                                )
                            else:
                                ...
                                print(
                                    f"Le joueur {player.first_name} {player.last_name}"
                                    f"a été ajouté au tournoi '{tournament_name}'."
                                )
                        else:
                            print(
                                f"Aucun joueur avec l'identifiant {chess_id} n'a été trouvé dans la base de données."
                            )
                    else:
                        print(
                            f"Tournoi '{tournament_name}' introuvable. Vérifiez le nom et réessayez."
                        )
                    ...
                    if tournament:
                        ...
                        if round_instance and round_instance.matches:
                            round_controller.enter_match_results(tournament, round_number, tournament_controller)
